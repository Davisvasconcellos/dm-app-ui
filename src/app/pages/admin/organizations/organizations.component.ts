  ...
  template: `
    <div class="p-6">
      ...
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-6">
                    ...
                    <table class="w-full text-sm">
                      <thead class="bg-gray-50 dark:bg-white/[0.02]">
                        <tr>
                          ...
                          <th class="px-4 py-2 text-right font-medium text-gray-500 dark:text-gray-400">Ações</th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                          ...
                          <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-3">
                              <button *ngIf="invite.status === 'pending' && !isExpired(invite.expires_at) && getInviteFromCache(invite.id_code)" 
                                ...
                                class="text-gray-400 hover:text-brand-500" title="Copiar Link">
                              ...
                              <button *ngIf="invite.status === 'pending'" (click)="openRevokeInviteModal(st.id_code, invite)" 
                                class="text-gray-400 hover:text-error-500 transition-colors" title="Revogar Convite">
                                <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                              </button>
                            </div>
                          </td>
        ...
        <app-modal [isOpen]="isDeleteCollaboratorModalOpen" (close)="cancelDeleteCollaborator()">
          <div class="p-6 text-center">
            <div class="mx-auto mb-4.5 flex h-17.5 w-17.5 items-center justify-center rounded-full bg-red-100 dark:bg-red-500/10">
              <svg class="size-7.5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
              </svg>
            </div>
            <h3 class="mb-2 text-xl font-bold text-gray-800 dark:text-white">Remover Colaborador</h3>
            <p class="mb-7.5 text-base text-gray-600 dark:text-gray-400">
              Tem certeza que deseja remover o colaborador <strong>{{ collaboratorToDelete?.email || collaboratorToDelete?.invited_email }}</strong>?<br>
              Esta ação não pode ser desfeita.
            </p>
            <div class="flex items-center justify-center gap-4.5">
              <button (click)="cancelDeleteCollaborator()" class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-100 dark:bg-gray-800 dark:text-white dark:border-gray-700 dark:hover:bg-gray-700">
                Cancelar
              </button>
              <button (click)="confirmDeleteCollaborator()" [disabled]="isSubmitting" class="flex w-full justify-center rounded-lg bg-red-500 px-6 py-2 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-50">
                Remover
              </button>
            </div>
          </div>
...
export class OrganizationsComponent implements OnInit {
  ...
  openDeleteCollaboratorModal(storeId: string, colab: any) {
    if (!colab.member_id) {
      this.toast.triggerToast('warning', 'Atenção', 'Este colaborador não possui um vínculo ativo identificado.');
      return;
    }
    this.collaboratorToDelete = colab;
    this.storeIdForDeletion = storeId;
    this.isRevokingInvite = false;
    this.isDeleteCollaboratorModalOpen = true;
  }

  openRevokeInviteModal(storeId: string, invite: StoreInvite) {
    this.collaboratorToDelete = { ...invite, email: invite.invited_email };
    this.storeIdForDeletion = storeId;
    this.isRevokingInvite = true;
    this.isDeleteCollaboratorModalOpen = true;
  }
  ...
  cancelDeleteCollaborator() {
    ...
    this.storeIdForDeletion = '';
  }

  confirmDeleteCollaborator() {
    if (!this.collaboratorToDelete || !this.storeIdForDeletion) return;

    this.isSubmitting = true;
    const request$ = this.isRevokingInvite
      ? this.storeInviteService.revokeInvite(this.collaboratorToDelete.id_code, this.storeIdForDeletion)
      : this.storeService.removeStoreMember(this.storeIdForDeletion, this.collaboratorToDelete.member_id);

    request$.subscribe({
      next: () => {
        const msg = this.isRevokingInvite ? 'Convite revogado.' : 'Colaborador removido.';
        this.toast.triggerToast('success', 'Sucesso', msg);
        this.fetchStoreUsers(this.storeIdForDeletion, true);
        this.cancelDeleteCollaborator();
        this.isSubmitting = false;
      },
      error: () => {
        this.isSubmitting = false;
        this.toast.triggerToast('error', 'Erro', 'Falha ao processar solicitação.');
      }
    });
  }

  // Métodos antigos mantidos/mapeados por segurança se utilizados em algum lugar dinâmico
  removeCollaborator(storeId: string, colab: any) { this.openDeleteCollaboratorModal(storeId, colab); }
  revokePendingInvite(storeId: string, invite: StoreInvite) { this.openRevokeInviteModal(storeId, invite); }
}
